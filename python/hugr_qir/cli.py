...
def _unexpected_failure(details: str) -> str:
    return (
        f"Compilation failed unexpectedly: {details}\n"
        "This may be a compiler bug. Please report it with a reproducing example."
    )


def _run_compiler(*args: str) -> None:
    try:
        cli(*args)
        ...
        # PyO3's PanicException inherits directly from BaseException.
        raise ValueError(_unexpected_failure(str(e))) from e
...
def hugr_qir_impl(  # noqa: PLR0913, PLR0917
    ...
    with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as tmp_dir:
        ...
        _run_compiler(str(hugr_file), *tmp_options)
        try:
    ...
    if validate_qir:
        try:
            ...
            if "__quantum__rt__qubit_release" in error_message and target == "native":
                ...
                msg = (
                    "Compilation failed: Generated QIR failed validation: "
                    f"{error_message}"
                )
...
def tmp_file_settings(output_format: OutputFormat) -> tuple[str, str]:
    ...
    for reading the temporary file generated by the rust code."""
    match output_format:
