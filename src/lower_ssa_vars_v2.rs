...
fn module_has_lowerable_values(module: &Module) -> bool {
    module
        .get_functions()
        .any(|func| !analyze_function(func).unwrap_or_default().is_empty())
}
...
fn trace_qubit_value(inst: InstructionValue, lowerable: &mut HashSet<ValueKey>) {
...
fn detect_loops(func: FunctionValue) -> Result<()> {
    fn dfs(block: BasicBlock, colors: &mut HashMap<ValueKey, u8>) -> Result<()> {
        ...
        match colors.get(&key).copied() {
            Some(1) => bail!(
                "lower_ssa_vars_v2 requires DAG CFGs; detected a cycle at block {}",
                name_of_block(block)
            ),
            Some(2) => return Ok(()),
            _ => {}
...
// ═══════════════════════════════════════════════════════════════════════════
// Utilities (copied from lower_ssa_vars.rs)
// ═══════════════════════════════════════════════════════════════════════════
