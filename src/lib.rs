...
use inkwell::values::{AsValueRef, BasicValue, BasicValueEnum, FunctionValue, InstructionValue, Operand, PhiValue, ValueKind};
...
impl CompileArgs {
    ...
    fn simplify_cfg(&self, module: &Module) -> Result<()> {
        ...
        let mut opt_str = String::from("simplifycfg");
        let _ = module.run_passes(opt_str.as_str(), &ctm, PassBuilderOptions::create());
        Ok(())
    }
...
pub fn replace_select_phi_on_qubit(module: &Module) -> bool {
    ...
    for block in first_func.get_basic_blocks() {
        ...
        while let Some(inst) = inst_opt {
            ...
            if inst.get_opcode() != InstructionOpcode::Phi { break; }
            ...
            if let BasicTypeEnum::PointerType(ptr_ty) = phi.as_basic_value().get_type() {
                if ptr_ty.get_element_type().into_struct_type().get_name().unwrap().eq(c"Qubit"){
        ...
        println!("phi preds {:?}", preds.iter().map(|b| b.get_name()).collect::<Vec<_>>());
        ...
        for pred in preds {
            ...
            let clone_block = pred.get_context().insert_basic_block_after(pred, &format!("{}_dup", name_of_block(block)));
            ...
            for phi in &phi_candidates {
                if let Some((val, inc_bb)) = incoming_for_predecessor(*phi, pred) {
                    let key = phi.as_instruction().get_name().unwrap().to_string_lossy().to_string();
                    vmap.insert(key, val);
                } else {
            ...
            match term.get_opcode() {
                Op::Br => {
                    ...
                    if !term.is_conditional() {
                        let block = operand_as_bb(term, 0);
...
fn first_non_phi<'ctx>(bb: BasicBlock<'ctx>) -> Option<InstructionValue<'ctx>> {
...
fn name_of_value(v: InstructionValue<'_>) -> String {
    v.get_name().unwrap().to_string_lossy().to_string()
}
...
fn incoming_for_predecessor<'ctx>(phi: PhiValue<'ctx>, pred: BasicBlock<'ctx>)
...
fn rebuild_tail_into<'ctx>(
    ...
    while let Some(inst) = it {
        if inst.is_terminator() {
            if !rebuild_terminator(builder, inst, vmap) { return false; }
            break;
        }
        match rebuild_inst2(builder, inst, vmap) {
...
fn remap<'ctx>(vmap: &HashMap<String, BasicValueEnum<'ctx>>, v: BasicValueEnum<'ctx>) -> BasicValueEnum<'ctx> {
    if let Some(orig) = v.as_instruction_value().map(|iv| iv.get_name().unwrap().to_string_lossy().to_string()) {
    ...
    match inst.get_opcode() {
        Op::Br => {
            if inst.is_conditional() {
                let cond = remap(vmap,  inst_operand_value(inst, 0).unwrap().into()).into_int_value();
        ...
        Op::Return => {
            let val = inst.get_operand(0).and_then(|o| operand_as_value(o)).map(|v| remap(vmap, v.into()));

            use inkwell::values::BasicValue; // bring the trait into scope
            let ok = builder
                .build_return(
                    val.as_ref()
                        .map(|v| v as &dyn BasicValue)  // coerce &BasicValueEnum -> &dyn BasicValue
                )
                .is_ok();
            ok
        }
        ...
        match term.get_opcode() {
            Op::Br => {
                if term.is_conditional() {
                    ...
                    builder.build_conditional_branch(cond, new_else, new_then).ok();
...
pub fn rebuild_inst2<'ctx>(
    ...
    match inst.get_opcode() {
        ...
        Op::GetElementPtr => unsafe {
            ...
            let built = builder.build_gep(base.into_pointer_value(), &indices, &name).ok().ok_or(())?;
        ...
        Op::BitCast => {
            ...
            match dst_any.try_into() {
                Ok(inkwell::types::BasicTypeEnum::PointerType(dst_ptr_ty)) => {
                    let src_ptr = match src_val { BasicValueEnum::PointerValue(p) => p, _ => return Err(()), };
                    let cast = builder.build_pointer_cast(src_ptr, dst_ptr_ty, &name).ok().ok_or(())?;
                    Ok(RebuildOutcome::Value(cast.as_basic_value_enum()))
                }
                Ok(inkwell::types::BasicTypeEnum::IntType(dst_int_ty)) => {
                    let src_int = match src_val { BasicValueEnum::IntValue(i) => i, _ => return Err(()), };
                    if src_int.get_type().get_bit_width() != dst_int_ty.get_bit_width() { return Err(()); }
                    let cast = builder.build_int_cast(src_int, dst_int_ty, &name).ok().ok_or(())?;
                    Ok(RebuildOutcome::Value(cast.as_basic_value_enum()))
                }
                Ok(inkwell::types::BasicTypeEnum::FloatType(dst_fp_ty)) => {
                    let src_fp = match src_val { BasicValueEnum::FloatValue(f) => f, _ => return Err(()), };
                    if src_fp.get_type() != dst_fp_ty { return Err(()); }
                    let cast = builder.build_float_cast(src_fp, dst_fp_ty, &name).ok().ok_or(())?;
                    Ok(RebuildOutcome::Value(cast.as_basic_value_enum()))
                }
                _ => Err(()),
            }
        ...
        Op::ICmp => {
            ...
            let cmp = builder.build_int_compare(pred, lhs, rhs, &name).ok().ok_or(())?;
        ...
        Op::Select => {
            ...
            let sel = builder.build_select(cond, tval, fval, &name).ok().ok_or(())?;
            Ok(RebuildOutcome::Value(sel))
        }

        // ---------------- Integer arithmetic ----------------
        Op::Add | Op::Sub | Op::Mul | Op::UDiv | Op::SDiv | Op::URem | Op::SRem | Op::And | Op::Or | Op::Xor => {
            let lhs = remap(vmap, inst_operand_value(inst, 0).ok_or(())?).into_int_value();
            let rhs = remap(vmap, inst_operand_value(inst, 1).ok_or(())?).into_int_value();
...
pub fn rebuild_inst<'ctx>(
    ...
    println!("opcode: {:?}", inst.get_opcode());
    ...
    match inst.get_opcode() {
        ...
        Op::GetElementPtr => unsafe {
            ...
            let base = remap(
                vmap,
                inst_operand_value(inst, 0)?.into(), // operand 0 is the pointer
            );
            // Indices follow; number depends on the original GEP
            // We need to recover the typed indices from the operands.
            let mut indices = Vec::new();
            // Heuristic: for GEP, operands after 0 are indices.
            // If your IR creates GEPs with a known count, you can also query it via the GEP wrapper;
            // but for simplicity, we pull all remaining operands as ints.
            let num_ops = inst.get_num_operands();
            for i in 1..num_ops {
                if let Some(idx_val) = inst_operand_value(inst, i) {
                    indices.push(remap(vmap, idx_val).into_int_value());
                }
            }
            let name = inst.get_name().unwrap().to_string_lossy();
            let built = builder
                .build_gep(base.into_pointer_value(), &indices, &name)
                .ok()?;
            Some(built.as_basic_value_enum())
        }

        Op::BitCast => {
            // %dst = bitcast <src> to <dstty>
            let src = remap(vmap, inst_operand_value(inst, 0)?);
            let dst_ty: BasicTypeEnum = inst.get_type().try_into().unwrap(); // LLVM 14 still has typed pointers
            let name = inst.get_name().unwrap().to_string_lossy();
            let built = builder.build_bit_cast(src, dst_ty, &name).ok()?;
            Some(built.as_basic_value_enum())
        }
        ...
        Op::Load => {
            // load <ty>, <ty>* %addr
            let addr = remap(vmap, inst_operand_value(inst, 0)?).into_pointer_value();
            let name = inst.get_name().unwrap().to_string_lossy();
            // The load's result type is carried on the instruction
            let built = builder.build_load(addr, &name).ok()?;
            Some(built)
        }

        Op::Store => {
            // store <ty> %val, <ty>* %addr
            let val = remap(vmap, inst_operand_value(inst, 0)?);
            let addr = remap(vmap, inst_operand_value(inst, 1)?).into_pointer_value();
            builder.build_store(addr, val).ok()?;
            None
        }
        ...
        Op::ICmp => {
            ...
            let lhs = remap(vmap, inst_operand_value(inst, 0)?).into_int_value();
            let rhs = remap(vmap, inst_operand_value(inst, 1)?).into_int_value();
            let name = inst.get_name().unwrap().to_string_lossy();
            let built = builder.build_int_compare(pred, lhs, rhs, &name).ok()?;
            Some(built.as_basic_value_enum())
        }

        Op::Select => {
            let cond = remap(vmap, inst_operand_value(inst, 0)?).into_int_value();
            let tval = remap(vmap, inst_operand_value(inst, 1)?);
            let fval = remap(vmap, inst_operand_value(inst, 2)?);
            let name = inst.get_name().unwrap().to_string_lossy();
            let built = builder.build_select(cond, tval, fval, &name).ok()?;
            Some(built)
        }
        ...
        Op::Add | Op::Sub | Op::Mul | Op::UDiv | Op::SDiv | Op::URem | Op::SRem | Op::And | Op::Or | Op::Xor => {
            ...
            let rhs = remap(vmap, inst_operand_value(inst, 1)?).into_int_value();
            let name = inst.get_name().unwrap().to_string_lossy();
            let built = match inst.get_opcode() {
                Op::Add  => builder.build_int_add(lhs, rhs, &name).ok()?,
                Op::Sub  => builder.build_int_sub(lhs, rhs, &name).ok()?,
                Op::Mul  => builder.build_int_mul(lhs, rhs, &name).ok()?,
                Op::UDiv => builder.build_int_unsigned_div(lhs, rhs, &name).ok()?,
                Op::SDiv => builder.build_int_signed_div(lhs, rhs, &name).ok()?,
                Op::URem => builder.build_int_unsigned_rem(lhs, rhs, &name).ok()?,
                Op::SRem => builder.build_int_signed_rem(lhs, rhs, &name).ok()?,
                Op::And  => builder.build_and(lhs, rhs, &name).ok()?,
                Op::Or   => builder.build_or(lhs, rhs, &name).ok()?,
                Op::Xor  => builder.build_xor(lhs, rhs, &name).ok()?,
                _ => unreachable!(),
            };
            Some(built.as_basic_value_enum())
        }

        // -------- Calls (LLVM 14 path uses build_direct_call) --------
        Op::Call => {
            let cs = CallSiteValue::try_from(inst).unwrap();
            let callee = cs.get_called_fn_value()?;
            // Collect actual arguments from operands that are values (calls don’t have block operands).
            let mut args = Vec::new();
            for i in 0..cs.count_arguments(){
                if let Some(v) = inst_operand_value(inst, i) {
                    args.push(remap(vmap, v).into());
            ...
            //println!("inst {:?}", inst);
            let name = inst.get_name().unwrap_or(c"").to_string_lossy();
            let callsite = builder.build_call(callee, &args, &name).ok()?; // LLVM 14
            // If callee is void, there is no BasicValue result.
            let value = callsite.try_as_basic_value();
            match value {
        ...
        // -------- Not handled here --------
        // Extend with more opcodes as needed (SExt, ZExt, PtrToInt, IntToPtr, FP ops, ExtractValue, etc).
        _ => None,
    }
}
